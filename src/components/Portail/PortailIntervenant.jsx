  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...
        createdAt: new Date(),
        contenuPedagogique: form.contenuPedagogique.trim(),
        ...(form.syllabusURL  && { syllabusURL: form.syllabusURL, syllabusNom: form.syllabusNom }),
        ...(form.ficheSeanceURL && { ficheSeanceURL: form.ficheSeanceURL, ficheSeanceNom: form.ficheSeanceNom }),
      };
    ...
    <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-end sm:items-center justify-center p-0 sm:p-4">
      <div className="bg-white rounded-t-2xl sm:rounded-2xl w-full sm:max-w-lg max-h-[95vh] flex flex-col overflow-hidden shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto px-5 py-5 space-y-4">
          ...
          {/* Contenu pédagogique */}
          <div>
            <label className={labelCls}>Contenu pédagogique / Plan du cours</label>
            <textarea
          ...
          <div className="space-y-2">
            <label className={labelCls}>Documents pédagogiques</label>
              ...
              <div className="flex-1 min-w-0">
                ...
                {form.syllabusURL ? (
                  <a href={form.syllabusURL} target="_blank" rel="noopener noreferrer"
                    ...
                    {form.syllabusNom || 'Voir'}
                  </a>
                ) : (
                  <p className="text-xs text-slate-400">Aucun fichier</p>
                )}
              </div>
              <input ref={syllabusInputRef} type="file" accept=".pdf,.doc,.docx,.ppt,.pptx" className="hidden"
                onChange={e => {
                  const file = e.target.files?.[0];
                  if (file) uploadFile(file, 'syllabus', setUploadingSyllabus, 'syllabusURL', 'syllabusNom');
                  e.target.value = '';
                }} />
              <button type="button" disabled={uploadingSyllabus} onClick={() => syllabusInputRef.current?.click()}
              ...
              <div className="flex-1 min-w-0">
                ...
                {form.ficheSeanceURL ? (
                  <a href={form.ficheSeanceURL} target="_blank" rel="noopener noreferrer"
                    className="text-xs text-[#005989] underline truncate block">
                    {form.ficheSeanceNom || 'Voir'}
                  </a>
                ) : (
                  <p className="text-xs text-slate-400">Aucun fichier</p>
                )}
              </div>
              <input ref={ficheInputRef} type="file" accept=".pdf,.doc,.docx,.ppt,.pptx" className="hidden"
                onChange={e => {
                  const file = e.target.files?.[0];
                  if (file) uploadFile(file, 'fiches', setUploadingFiche, 'ficheSeanceURL', 'ficheSeanceNom');
                  e.target.value = '';
                }} />
              <button type="button" disabled={uploadingFiche} onClick={() => ficheInputRef.current?.click()}
                className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold border border-[#005989] text-[#005989] rounded-lg hover:bg-[#005989]/5 transition-colors disabled:opacity-50 shrink-0">
                {uploadingFiche
                  ? <svg className="w-3.5 h-3.5 animate-spin" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8H4z"/></svg>
                  : <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"/></svg>}
...
// ── Badges tab ─────────────────────────────────────────────────────────────────
function BadgesIntervenantTab({ intervenant, groupes, sessions }) {
  const [students, setStudents]     = useState([]);
  ...
  // Load students from intervenant's groups
  useEffect(() => {
    if (!intervenant?.id) return;
    (async () => {
      const myGroupIds = [...new Set(sessions.map(s => s.groupeId).filter(Boolean))];
      if (myGroupIds.length === 0) return;
      try {
        const all = [];
        for (const gid of myGroupIds) {
          const q = query(collection(db, 'students'), where('groupeId', '==', gid));
  ...
  // Load badges for selected student
  useEffect(() => {
    if (!selectedStudentId) { setStudentBadges([]); return; }
    setLoadingBadges(true);
    badgesService.getByStudent(selectedStudentId)
      .then(list => setStudentBadges(list))
      .catch(() => {})
      .finally(() => setLoadingBadges(false));
  }, [selectedStudentId]);

  const handleAward = async () => {
    if (!selectedStudentId || !selectedKey) return;
    setAwarding(true);
    try {
      await badgesService.award(selectedStudentId, selectedKey, {
        source: 'manual',
        attribueParId: intervenant?.id,
        attribueParNom: `${intervenant?.prenom || ''} ${intervenant?.nom || ''}`.trim(),
        note,
      });
      setAwarded(true);
      toast?.success?.('Badge attribué avec succès !');
      // Refresh
      const updated = await badgesService.getByStudent(selectedStudentId);
      setStudentBadges(updated);
      setSelectedKey(''); setNote('');
  ...
  const handleRevoke = async (badgeId) => {
    ...
    await badgesService.revoke(badgeId);
    const updated = await badgesService.getByStudent(selectedStudentId);
    setStudentBadges(updated);
    toast?.success?.('Badge retiré.');
  };

  const manualBadges = BADGE_CATALOGUE.filter(b => b.categorie === 'comportement');
  const selectedStudent = students.find(s => s.id === selectedStudentId);

  return (
    <div className="space-y-5">
      <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
        <div className="px-5 py-4 border-b border-slate-100" style={{ background: `${BLUE}07` }}>
          <p className="text-sm font-bold text-slate-800">Attribuer un badge</p>
          <p className="text-xs text-slate-400 mt-0.5">Récompensez et encouragez vos apprenants</p>
        </div>
        <div className="p-5 space-y-4">
          {/* Student picker */}
          <div>
            <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5">Apprenant</label>
            <select
              value={selectedStudentId}
              onChange={e => setSelectedStudentId(e.target.value)}
              className="w-full px-3.5 py-2.5 border border-slate-200 rounded-xl text-sm text-slate-800 focus:outline-none bg-white"
            >
              <option value="">— Sélectionner un apprenant —</option>
              {students.map(s => (
                <option key={s.id} value={s.id}>{s.prenom} {s.nom} {s.codeApprenant ? `(${s.codeApprenant})` : ''}</option>
              ))}
            </select>
          </div>
          ...
          {selectedStudentId && (
            <>
              <div>
                <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">Badge à attribuer</label>
                <div className="grid grid-cols-2 gap-2">
                  {manualBadges.map(b => (
                    <button key={b.key}
                      onClick={() => setSelectedKey(k => k === b.key ? '' : b.key)}
                      className="flex items-center gap-2 px-3 py-2.5 rounded-xl border text-sm font-semibold text-left transition-all"
                      style={{
                        borderColor: selectedKey === b.key ? b.border : '#e2e8f0',
                        background: selectedKey === b.key ? b.bg : '#f8fafc',
                        color: selectedKey === b.key ? b.text : '#64748b',
                        boxShadow: selectedKey === b.key ? `0 0 0 2px ${b.border}` : '',
                      }}
                    >
                      <span className="text-xl leading-none">{b.emoji}</span>
                      <span className="text-xs font-bold">{b.titre}</span>
                    </button>
                  ))}
                </div>
              </div>
              ...
              {/* Optional note */}
              <div>
                <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5">Commentaire (optionnel)</label>
                <textarea
                  value={note}
                  onChange={e => setNote(e.target.value)}
                  placeholder="Ex : Très bonne initiative lors du projet logistique…"
                  rows={2}
                  className="w-full px-3.5 py-2.5 border border-slate-200 rounded-xl text-sm text-slate-800 focus:outline-none resize-none"
                />
              </div>

              <button
                onClick={handleAward}
                disabled={!selectedKey || awarding}
                className="w-full py-3 rounded-xl text-white text-sm font-bold transition-all disabled:opacity-50"
                style={{ background: BLUE }}
              >
                {awarding ? 'Attribution…' : awarded ? '✓ Badge attribué !' : 'Attribuer le badge'}
              </button>
            </>
          )}
        </div>
      ...
      {/* Current badges for selected student */}
      {selectedStudentId && (
        <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
          <div className="px-5 py-3.5 border-b border-slate-100">
            <p className="text-sm font-semibold text-slate-800">
              Badges de {selectedStudent?.prenom} {selectedStudent?.nom}
            </p>
          </div>
          <div className="p-5">
            {loadingBadges ? (
              <div className="flex justify-center py-4">
                <div className="w-6 h-6 rounded-full border-2 animate-spin" style={{ borderColor: `${BLUE}30`, borderTopColor: BLUE }} />
              </div>
            ) : studentBadges.length === 0 ? (
              <p className="text-sm text-slate-400 text-center py-4">Aucun badge pour cet apprenant.</p>
            ) : (
              <div className="space-y-2">
                {studentBadges.map(badge => {
                  ...
                  return (
                    <div key={badge.id}
                      ...
                      style={{ background: def.bg || '#f8fafc', borderColor: def.border || '#e2e8f0' }}
                    >
                      <span className="text-xl leading-none">{badge.emoji}</span>
                      <div className="flex-1 min-w-0">
                        <p className="text-xs font-bold" style={{ color: def.text || '#1e293b' }}>{badge.titre}</p>
                        <p className="text-xs text-slate-400">{badge.source === 'manual' ? `Par ${badge.attribueParNom}` : 'Auto'}{badge.note ? ` · ${badge.note}` : ''}</p>
                      </div>
                      {badge.source === 'manual' && badge.attribueParId === intervenant?.id && (
                        <button onClick={() => handleRevoke(badge.id)} className="text-xs text-red-400 hover:text-red-600 transition-colors shrink-0">
                          Retirer
                        </button>
                      )}
                    </div>
      ...
      <div className="flex-1 lg:ml-64 flex flex-col min-h-screen">
        ...
        <main className="flex-1 px-4 lg:px-8 pb-24 lg:pb-10 pt-4 lg:pt-0">
          <div className="max-w-3xl">
            ...
            {activeTab === 'badges' && (
              <BadgesIntervenantTab intervenant={intervenant} groupes={groupes} sessions={sessions} />
            )}
